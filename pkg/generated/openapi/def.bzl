...
def openapi_library(name, tags, srcs, openapi_targets=[], vendor_targets=[]):
  ...
  ] + ["//%s:go_default_library" % target for target in openapi_targets]
  go_library(
      name=name,
      tags=tags,
  ...
  go_genrule(
      ...
      cmd = " ".join([
        ...
        "--output-package k8s.io/federation/pkg/generated/openapi",
        "--input-dirs " + ",".join(["k8s.io/federation/" + target for target in openapi_targets]),
