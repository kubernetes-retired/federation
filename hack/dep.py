...
f8n_dir = join(os.environ['GOPATH'], 'src', 'k8s.io', 'federation')
...
# with open(join(f8n_dir, 'dep-status.json')) as data_file:
#   dep_status = json.load(data_file)

process = subprocess.Popen(['dep', 'status', '-json'], stdout=subprocess.PIPE, cwd=f8n_dir)
...
for dep in dep_status:
  ...
  dep_status_ver_map[dep['ProjectRoot']] = dep['Version']
...
for dep in dep_status_map:
  ...
  if godep_sha:
    ...
    print("# Missing dep %s, currently at version %s" % (dep, dep_status_ver_map.get(dep)))
