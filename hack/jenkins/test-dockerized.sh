...
# kubekins-test container with a federation repo mapped in. See
...
cd /go/src/k8s.io/federation

make generated_files
# TODO(marun) not necessary so long as test-cmd isn't running
# go install ./cmd/...
./hack/install-etcd.sh

make test
# TODO(marun) Need to source a kubectl binary since it can't be built
