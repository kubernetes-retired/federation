...
function dump_federation_pod_logs() {
  local -r federation_pod_names_string="$(${REAL_KUBE_ROOT}/cluster/kubectl.sh get pods -l 'app=federated-cluster' --namespace=${FEDERATION_NAMESPACE} -o name)"
...
function dump_dns_pod_logs() {
  local -r dns_pod_names_string="$(${REAL_KUBE_ROOT}/cluster/kubectl.sh get pods -l 'k8s-app=kube-dns' --namespace=kube-system -o name)"
